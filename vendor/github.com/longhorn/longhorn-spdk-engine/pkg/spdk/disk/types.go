...
func getDiskDriverForBDF(diskDriver commonTypes.DiskDriver, bdf string) (commonTypes.DiskDriver, error) {
	...
	output, err := spdksetup.GetDiskStatus(bdf, executor)
	if err != nil {
		return "", errors.Wrapf(err, "failed to get disk status for BDF %s", bdf)
	}

	var diskStatus DiskStatus
	err = json.Unmarshal([]byte(output), &diskStatus)
	if err != nil {
		return "", errors.Wrapf(err, "failed to unmarshal disk status for BDF %s", bdf)
	}
...
func getDriverForAuto(diskStatus DiskStatus, diskPath string) (commonTypes.DiskDriver, error) {
