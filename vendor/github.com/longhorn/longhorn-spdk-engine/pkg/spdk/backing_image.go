...
func (bi *BackingImage) Create(spdkClient *spdkclient.Client, superiorPortAllocator *commonbitmap.Bitmap, fromAddress string, srcLvsUUID string) (ret *spdkrpc.BackingImage, err error) {
	...
	go func() {
		bi.prepareBackingImageSnapshot(spdkClient, superiorPortAllocator, fromAddress, srcLvsUUID)
...
func (bi *BackingImage) prepareBackingImageSnapshot(spdkClient *spdkclient.Client, superiorPortAllocator *commonbitmap.Bitmap, fromAddress string, srcLvsUUID string) (err error) {
	...
	if bi.LvsUUID != "" {
		lvsList, err = spdkClient.BdevLvolGetLvstore("", bi.LvsUUID)
	} else if bi.LvsName != "" {
		lvsList, err = spdkClient.BdevLvolGetLvstore(bi.LvsName, "")
	}
	if err != nil {
		return err
	}
	if len(lvsList) != 1 {
		return fmt.Errorf("found zero or multiple lvstore with name %s and UUID %s during backing image %s creation", bi.LvsName, bi.LvsUUID, bi.Name)
	}
	if bi.LvsName == "" {
		bi.LvsName = lvsList[0].Name
	}
	if bi.LvsUUID == "" {
		bi.LvsUUID = lvsList[0].UUID
	}
	if bi.LvsName != lvsList[0].Name || bi.LvsUUID != lvsList[0].UUID {
		return fmt.Errorf("found mismatching between the actual lvstore name %s with UUID %s and the recorded lvstore name %s with UUID %s during backing image %s creation", lvsList[0].Name, lvsList[0].UUID, bi.LvsName, bi.LvsUUID, bi.Name)
	}
	bi.log = logrus.StandardLogger().WithFields(logrus.Fields{
		"backingImagename": bi.Name,
		"lvsName":          bi.LvsName,
	...
	if isSourceFromBIM {
		...
		if err := bi.prepareFromSync(headFh, fromAddress, srcLvsUUID); err != nil {
			bi.log.WithError(err).Warnf("Failed to prepare the backing image %v by syncing from %v and srcLvsUUID", bi.Name, fromAddress, srcLvsUUID)
			return errors.Wrapf(err, "failed to prepare the backing image %v by syncing from  %v and srcLvsUUID", bi.Name, fromAddress, srcLvsUUID)
		}
	}
